...
const sendMessageToChat = () => {
    if (props.lobby) {
        storeChat.sendMessageToLobbyChat(message.value)
    } else {
...
<template>
    <Card class="my-8 py-2 px-1">
        <CardHeader class="pb-6">
            <CardTitle v-if="!lobby">Chat</CardTitle>
            <CardTitle v-else>Chat of Lobby</CardTitle>
            <CardDescription>
        ...
        <CardContent class="p-4">
            <div class="divide-y divide-solid divide-gray-200">
                ...
                <div v-else>
                    <div v-if="storeChat.totalLobbyMessages > 0">
                        <div v-for="messageObj in storeChat.lobbyMessages" :key="messageObj" class="flex">
                            <div class="flex flex-col grow pb-6">
                                <div class="text-xs text-gray-500">
                                    <span :class="{
                                        'hover:text-green-300': canSendMessageToUser(messageObj.user),
