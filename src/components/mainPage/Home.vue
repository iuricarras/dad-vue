...
<template>
  <div class="flex flex-col lg:flex-row items-center justify-center mt-1 space-y-5 lg:space-y-0 lg:space-x-10">
    <div class="w-full lg:max-w-2xl lg:mr-10">
      <TopList />
    </div>

    <div class="flex flex-col space-y-5 w-full lg:w-1/4">
      <div v-if="!storeAuth.user || storeAuth.user.type !== 'A'"
        class="bg-blue-600 text-white p-6 rounded hover:bg-blue-500 transition-all w-full">
        <RouterLink to="/singleplayer" class="block text-center">
          ...
          <p class="text-sm lg:text-lg mt-2">
            Joga agora e desafia a tua memória!
          </p>
        </RouterLink>
      </div>

      <RouterLink to="/multiplayer" class="block text-center">
        <div v-if="!storeAuth.user || (storeAuth.user && storeAuth.user.type !== 'A')"
          class="bg-gray-600 text-white p-6 rounded w-full cursor-not-allowed">
          ...
          <p class="text-sm lg:text-lg text-center mt-2">
            Faz login para desbloquear este modo de jogo!
          </p>
        </div>
      </RouterLink>
